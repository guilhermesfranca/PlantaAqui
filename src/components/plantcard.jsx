...
export default function PlantCardGrid({ recentPlants, viewMode }) {
  return (
      ...
      {recentPlants.map((plant) => (
        ...
        >
          <div className={viewMode === 'list' ? 'w-20 h-20 flex-shrink-0 relative rounded-xl overflow-hidden' : 'relative h-48'}>
            ...
            />
            <button className="absolute top-2 right-2 w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-md hover:bg-white transition-colors">
              <svg
                className="w-4 h-4"
                viewBox="0 0 24 24"
